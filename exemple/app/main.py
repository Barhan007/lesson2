...
async def index() -> Dict[str, str]:
    ...
    try:
        ...
        logger.info(f"Успешное маршрута выполнение index пользователя для: {user}")
        return {"message": f"Приветствую тебя, землянин User: {user}"}
    except Exception as e:
        logger.error(f"Ошибка в маршруте index: {str(e)}")
        raise HTTPException(status_code=500, detail="Внутренняя сервера ошибка!!!")
...
async def health_check() -> Dict[str, str]:
    ...
    """
    logger.info("Успешно эндпоинт health_check вызван")
